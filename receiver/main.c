...
int main(int argc, char* argv[])
{
    ...
    // ARGUMENT COUNT = 2, IF NOT 2 SHOW HELP MESSAGE AND EXIT (FAIL)
    if (argc != 2)
    {
        printf("Usage: %s <port>\n", argv[0]);
        return EXIT_FAILURE;
    ...
    struct sockaddr_in my_addr = {.sin_family = AF_INET,
                                  .sin_addr.s_addr = INADDR_ANY, // LISTEN FOR PACKETS FROM ANY IP ADDRESS
    ...
    int result = bind(udp_recieve_socket, (struct sockaddr*)&my_addr, sizeof(my_addr));
    check(result, "ERROR: Could not bind socket to address");

    // RECEIVE!
    socklen_t address_length = sizeof(peer_addr);
    int bytes_received = recvfrom(udp_recieve_socket, buffer, BUFFER_SIZE, 0, (struct sockaddr*)&peer_addr, &address_length);
