...
int main(int argc, char* argv[])
{
    ...
    // ARGUMENT COUNT EXPECTED = 4, THEREFORE IF NOT 4 SHOW HELP MESSAGE AND EXIT (FAIL)
    if (argc != 4)
    {
    ...
    // ARGUMENTS PASSED IN WHEN RUNNING FILE
    ...
    struct sockaddr_in peer_adr = {
        .sin_family = AF_INET,       // Socket Internet Family | AF_INET = IPv4 address family, can use AF_INET6 for IPv6
        .sin_port = htons(peer_port) // Socket Internet Port | htons = Host to Network Short
    };
    ...
    */
    ...
    if (sendto(udp_socket, message, strlen(message) + 1, 0, (struct sockaddr*)&peer_adr, sizeof(peer_adr)) <= 0)
    {
